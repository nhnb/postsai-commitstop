...
export class AppComponent implements OnInit {
    ...
    currentConfig = signal(new Configuration("- .* .* .* .* .*", "comment"));

    rows = signal<Submission[]>([]);
    ...
    setRows(rows: Submission[]) {
        this.rows.set(rows);
        if (rows.length > 0) {
            this.currentConfig.set(rows[0].config.clone());
    ...
    onSelect() {
        if (this.selected.length > 0) {
            queueMicrotask(() => {
                this.currentConfig.set(this.selected[0].config.clone());
            });
        }
    }
    ...
    doSubmit() {
        if (this.rows().length > 0 && this.currentConfig().sameAs(this.rows()[0].config))
            ...
            this.configurationsService.saveConfig(added).subscribe((result) => {
                ...
                err => {
                    console.log(err);
                    alert("Could not save configuration: " + err.error);
                });
        }
    }
}
